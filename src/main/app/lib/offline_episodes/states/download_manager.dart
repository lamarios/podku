...
class DownloadManagerCubit extends Cubit<DownloadManagerState> with WidgetsBindingObserver {
  ...
  Future<void> download(Episode e, {required bool manualDownload, int retries = 0}) async {
    if (state.downloadStatus.containsKey(e.id)) {
      _log.fine("Download already queues: ${state.downloadStatus[e.id]?.status}");
      return;
    }

    if (retries > 2) {
      _log.fine('Too many retries to download episode ${e.id}, giving up...');
      return;
    }
    ...
    try {
      ...
      statuses[e.id!] = DownloadProgress(id: downloadTaskId, status: .enqueued, progress: 0, retries: retries);
...
*/
      final task = DownloadTask(
        ...
        directory: '${EpisodeDownloads.episodesFolder}/${e.id}',
