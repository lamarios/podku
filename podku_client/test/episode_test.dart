import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for Episode
void main() {
  final Episode? instance = /* Episode(...) */ null;
  // TODO add properties to the entity

  group(Episode, () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String title
    test('to test the property `title`', () async {
      // TODO
    });

    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // String audioUrl
    test('to test the property `audioUrl`', () async {
      // TODO
    });

    // String audioType
    test('to test the property `audioType`', () async {
      // TODO
    });

    // int audioLengthBytes
    test('to test the property `audioLengthBytes`', () async {
      // TODO
    });

    // int pubDateMillis
    test('to test the property `pubDateMillis`', () async {
      // TODO
    });

    // int durationSeconds
    test('to test the property `durationSeconds`', () async {
      // TODO
    });

    // double progress
    test('to test the property `progress`', () async {
      // TODO
    });

    // String guid
    test('to test the property `guid`', () async {
      // TODO
    });

    // String imageUrl
    test('to test the property `imageUrl`', () async {
      // TODO
    });

    // int seasonNumber
    test('to test the property `seasonNumber`', () async {
      // TODO
    });

    // int episodeNumber
    test('to test the property `episodeNumber`', () async {
      // TODO
    });

    // String episodeType
    test('to test the property `episodeType`', () async {
      // TODO
    });

    // bool explicit
    test('to test the property `explicit`', () async {
      // TODO
    });

    // String link
    test('to test the property `link`', () async {
      // TODO
    });

    // bool processed
    test('to test the property `processed`', () async {
      // TODO
    });

    // int timeUpdated
    test('to test the property `timeUpdated`', () async {
      // TODO
    });

    // List<Chapter> chapters
    test('to test the property `chapters`', () async {
      // TODO
    });

    // List<EpisodeFile> files
    test('to test the property `files`', () async {
      // TODO
    });

    // List<EpisodePerson> people
    test('to test the property `people`', () async {
      // TODO
    });

    // PodcastLight podcast
    test('to test the property `podcast`', () async {
      // TODO
    });

  });
}
