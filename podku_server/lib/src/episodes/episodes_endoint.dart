...
class EpisodesEndpoint extends Endpoint {
  ...
  Future<void> startPlayback(Session session, Episode episode, UuidValue player) async {
    session.log('Starting playback of episode: ${episode.title}');
    await session.messages.postMessage(
      _progressChannel,
      EpisodeProgress(episodeId: episode.id, progress: episode.progress, player: player, newPlayback: true),
    );
  }

  Future<void> setProgress(Session session, Episode episode, UuidValue player) async {
